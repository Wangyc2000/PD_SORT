...
class KalmanFilterNew(object):
    ...
    **Example**
    ...
    def apply_affine_correction(self, m, t, img_h):
        """
        Apply to both last state and last observation for OOS smoothing.

        Messy due to internal logic for kalman filter being messy.
        """
        # x_ori:[x, y, s, r, vx, vy, vs]
        # x: [x, y,pd, s, r,vx,vy,vpd,vs]

        scale = np.linalg.norm(m[:, 0])
        self.x[:2] = m @ self.x[:2] + t  # 修正中心点坐标
        self.x[5:7] = m @ self.x[5:7]  # 修正vx，vy

        # ** 修正pd **
        # w = np.sqrt(self.x[3] * self.x[4])
        # h = self.x[3] / w
        # y2 = self.x[1] + h/2.
        # self.x[2] = 2.*img_h - y2

        self.P[:2, :2] = m @ self.P[:2, :2] @ m.T
        self.P[5:7, 5:7] = m @ self.P[5:7, 5:7] @ m.T
        ...
        if not self.observed and self.attr_saved is not None:
            ...
            self.attr_saved["x"][5:7] = m @ self.attr_saved["x"][5:7]
            # ** pd **
            # self.attr_saved["x"][2] = 2.*img_h - (self.attr_saved["x"][1] + (self.attr_saved["x"][3]/np.sqrt(self.attr_saved["x"][3] * self.attr_saved["x"][4]))/2.)
            self.attr_saved["P"][:2, :2] = m @ self.attr_saved["P"][:2, :2] @ m.T
            self.attr_saved["P"][5:7, 5:7] = m @ self.attr_saved["P"][5:7, 5:7] @ m.T
            self.attr_saved["last_measurement"][:2] = m @ self.attr_saved["last_measurement"][:2] + t


    def unfreeze(self):
        if self.attr_saved is not None:
            # history_obs中的元素 z:[x,y,pd,s,r]
            new_history = deepcopy(self.history_obs)
            self.__dict__ = self.attr_saved
            # self.history_obs = new_history 
            self.history_obs = self.history_obs[:-1]  # 取history_obs历史观测中除了最后一个元素（即当前帧（目标重现时）的观测）外的所有元素，在后面的update中会重新加入
            occur = [int(d is None) for d in new_history]  # 为none的是1，不为none的是0
            indices = np.where(np.array(occur)==0)[0]  # 取不为none的历史观测的索引
            index1 = indices[-2]  # 倒数第二个不为none的历史观测的索引
            index2 = indices[-1]  # 最后一个不为none的历史观测的索引（即当前帧（目标重现时）的观测）
            # box1 = new_history[index1]  # 最后一个不为none的观测
            box1 = self.last_measurement  # ** cmc **
            # x1, y1, s1, r1 = box1
            # ** 增加一维pd **
            x1, y1, pd1, s1, r1 = box1
            w1 = np.sqrt(s1 * r1)
            h1 = np.sqrt(s1 / r1)
            box2 = new_history[index2]  # 倒数第二个不为none的观测
            # x2, y2, s2, r2 = box2
            # ** 增加一维pd **
            x2, y2, pd2, s2, r2 = box2
            w2 = np.sqrt(s2 * r2)
            h2 = np.sqrt(s2 / r2)
            time_gap = index2 - index1  # 两次观测（最后一次和倒数第二次）之间间隔的帧数
            dx = (x2-x1)/time_gap  # 平均每帧间隔的x
            dy = (y2-y1)/time_gap  # 平均每帧间隔的y
            # ** 增加一维pd **
            dpd = (pd2-pd1)/time_gap
            dw = (w2-w1)/time_gap  # 平均每帧间隔的w
            dh = (h2-h1)/time_gap  # 平均每帧间隔的h
            for i in range(index2 - index1):
                ...
                """
                # 在最后一次与倒数第二次观测之间构建虚拟轨迹，其中每一帧构建一个虚拟的观测
                x = x1 + (i+1) * dx  # 当前帧的虚拟观测的x
                y = y1 + (i+1) * dy  # 当前帧的虚拟观测的y
                # ** 增加一维pd **
                pd = pd1 + (i+1) * dpd  # 当前帧的虚拟观测的pd
                w = w1 + (i+1) * dw   # 当前帧的虚拟观测的w
                h = h1 + (i+1) * dh  # 当前帧的虚拟观测的h
                s = w * h   # 当前帧的虚拟观测的s
                r = w / float(h)  # 当前帧的虚拟观测的r
                # new_box = np.array([x, y, s, r]).reshape((4, 1))
                # ** 增加一维pd **
                new_box = np.array([x, y, pd, s, r]).reshape((5, 1))  # 当前帧的虚拟观测
                """
                ...
                """
                self.update(new_box)  # 使用当前帧的虚拟观测更新卡尔曼滤波器
                if not i == (index2-index1-1):
                    self.predict()  # 执行卡尔曼滤波器的状态预测
                # if not i == (index2 - index1 - 1):
                #     self.update(new_box)
                #     self.predict()
                # else:
    ...
    def update(self, z, R=None, H=None):
        ...
        # 若当前帧没有成功观测则保存最后一次观测，并将自身状态置为self.observed = False
        if z is None:
            if self.observed:
                """
                ...
                """
                self.last_measurement = self.history_obs[-2]  # ** for CMC **
                self.freeze()
            self.observed = False 
            self.z = np.array([[None]*self.dim_z]).T
            self.x_post = self.x.copy()
        ...
        # 重新观测到则构建虚拟轨迹继续宁re-update，并并将自身状态置为self.observed = True
