...
def iou_batch_3d(dets, tracks):
    ...
    pd = np.maximum(0., pd)
    whd = w * h * pd
    union = ((bboxes1[..., 2] - bboxes1[..., 0]) * (bboxes1[..., 3] - bboxes1[..., 1]) * pd1
               + (bboxes2[..., 2] - bboxes2[..., 0]) * (bboxes2[..., 3] - bboxes2[..., 1]) * pd2 - whd)
    iou = whd / union
    return (iou)

    # GIoU
    # union_area = ((bboxes1[..., 2] - bboxes1[..., 0]) * (bboxes1[..., 3] - bboxes1[..., 1])
                  # + (bboxes2[..., 2] - bboxes2[..., 0]) * (bboxes2[..., 3] - bboxes2[..., 1]) - w * h)
    # xxc1 = np.minimum(bboxes1[..., 0], bboxes2[..., 0])
    # yyc1 = np.minimum(bboxes1[..., 1], bboxes2[..., 1])
    # xxc2 = np.maximum(bboxes1[..., 2], bboxes2[..., 2])
    # yyc2 = np.maximum(bboxes1[..., 3], bboxes2[..., 3])
    # pd2 = np.maximum(bboxes1[..., 5], bboxes2[..., 4])
    # wc = xxc2 - xxc1
    # hc = yyc2 - yyc1
    # assert (wc > 0).all()
    # assert (hc > 0).all()
    # assert (pd2 > 0).all()
    # volume_enclose = wc * hc * pd2
    # # volume_enclose = union_area * pd2
    # giou = iou - (volume_enclose - union) / volume_enclose
    # giou = (giou + 1.) / 2.0  # resize from (-1,1) to (0,1)
    # giou = iou + union/(wc * hc * pd2)
    # giou = union/volume_enclose
    # giou /= 2.
    # return (giou)
...
def associate(detections, trackers, iou_threshold, velocities, previous_obs, vdc_weight):
    ...
    # diff_direction = (np.pi / 2.0 - np.abs(diff_direction)) / np.pi  # 归一化角度
    # scores = np.repeat(detections[:, -2][:, np.newaxis], trackers.shape[0],
    #                    axis=1)  # 检测框的置信度分数（得detection中的倒数第二维，并拓展为矩阵）
    # valid_mask = np.ones(previous_obs.shape[0])
    # valid_mask[np.where(previous_obs[:, 4] < 0)] = 0  # <0应该是指为-1，即没有已记录的previous obs
