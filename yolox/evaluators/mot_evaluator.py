...
# from trackers.ocsort_tracker_cmc.ocsort import OCSort
from trackers.ocsort_tracker.ocsort import OCSort
# from trackers.ocsort_tracker_pd.ocsort import OCSort
...
class MOTEvaluator:
    ...
    def evaluate_ocsort(
        ...
        # ):
        # ** CMC 需要额外输入np_imgs **
        for cur_iter, ((imgs, np_imgs), _, info_imgs, ids) in enumerate(
                progress_bar(self.dataloader)
        ):
            with torch.no_grad():
                # init tracker
            ...
            if outputs[0] is not None:
                # online_targets = tracker.update(outputs[0], info_imgs, self.img_size)
                online_targets = tracker.update(outputs[0], imgs, np_imgs[0].numpy(), info_imgs, self.img_size, tag)  # for CMC
                online_tlwhs = []
                online_ids = []
                for t in online_targets:
