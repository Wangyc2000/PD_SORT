...
sys.path.append(r'/home/wangyc/My_OC_SORT')
...
def compare_dataframes(gts, ts):
    ...
    for k, tsacc in ts.items():
        if k in gts:       
            ...
            vflag = open("res_mot/eval_{}.txt".format(k), 'w')
...
def main(exp, args, num_gpu):
    ...
    if exp.val_ann == 'val_half.json':
        gt_type = '_val_half'
        # seqs = "MOT17-val"
        seqs = "MOT20-val" if args.mot20 else "MOT17-val"
    elif exp.val_ann == "train_half.json":
        gt_type = '_train_half'
        # seqs = "MOT17-train_half"
        seqs = "MOT20-train_half" if args.mot20 else "MOT17-train_half"
    elif exp.val_ann == "test.json":  # change to train.json when running on training set
        gt_type = ''
    ...
    *_, summary = evaluator.evaluate_ocsort(
    ...
    if args.mot20:
        gtfiles = glob.glob(
            os.path.join('datasets/MOT20/train', '*/gt/gt{}.txt'.format(gt_type)))  # MOT20
    else:
        gtfiles = glob.glob(
            os.path.join('datasets/mot/train', '*/gt/gt{}.txt'.format(gt_type)))  # MOT17
