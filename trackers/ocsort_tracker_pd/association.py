...
def cost_vel(Y, X, inertia_Y, inertia_X, detections, trackers, previous_obs, vdc_weight):
    ...
    scores = np.repeat(detections[:, -2][:, np.newaxis], trackers.shape[0], axis=1)  # 检测框的置信度分数（得detection中的倒数第二维，并拓展为矩阵）
...
def associate(detections, trackers, iou_threshold, velocities, previous_obs, vdc_weight):    
    ...
    Y, X, PD = speed_direction_batch(detections, previous_obs)  # 检测框中心点和轨迹中心点之间的方向向量（dy/斜边,dx/斜边，dpd/斜边）（num_track*num_det）
    # inertia_Y, inertia_X = velocities[:,0], velocities[:,1] # 轨迹中现有的最后两个检测框中心点之间的sin，cos值
    inertia_Y, inertia_X, inertia_PD = velocities[:,0], velocities[:,1], velocities[:,2]  # 轨迹中现有的最后两个检测框中心点之间的方向向量（dy/斜边,dx/斜边，dpd/斜边）
